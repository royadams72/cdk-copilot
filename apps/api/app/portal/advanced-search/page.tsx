    ...
    <section className={styles.subpageLayout}>
      ...
      <section className={styles.portalFormShellWide}>
        ...
        <div className={styles.portalButtonRow}>
          <button className={styles.buttonSecondarySmall} onClick={clearSearch} type="button">
            Clear
          </button>
          <button className={styles.buttonPrimarySmall} onClick={submitSearch} type="button">
            Search
          </button>
        </div>
      </section>
      ...
      <section className={styles.portalResultCard}>
        <div className={styles.portalResultHeader}>
          <h3 className={styles.carePlanPanelTitle}>Search results</h3>
          <p className={styles.dataScreenCaption}>
            {loading
              ? "Loading patients..."
              : `${matchedPatients} of ${totalPatients} patients matched.`}
          </p>
        </div>
        ...
        <div className={styles.portalFormSectionList}>
          {patients.map((patient) => (
            <div className={styles.portalResultRow} key={patient.id}>
              <div className={styles.portalResultCopy}>
                <strong>{patient.name}</strong>
                <span>
                  {[
                    patient.dateOfBirth ? `DOB ${patient.dateOfBirth}` : null,
                    patient.email,
                    patient.stage ? `Stage ${patient.stage}` : null,
                    patient.risk !== "unknown" ? `${patient.risk} risk` : "risk unknown",
                    patient.careTeamId ? `Team ${patient.careTeamId}` : null,
                    patient.facilityId ? `Facility ${patient.facilityId}` : null,
                  ]
                    .filter(Boolean)
                    .join(" · ")}
                </span>
