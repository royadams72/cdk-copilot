import type { TFoodNutrients, TOpenFoodFactsCandidate } from "@ckd/core";
...
): Promise<TOpenFoodFactsCandidate[]> {
  ...
  for (const baseUrl of OFF_BASE_URLS) {
    try {
      ...
      return (payload.products ?? [])
        ...
        .filter((candidate): candidate is TOpenFoodFactsCandidate => candidate !== null);
    } catch (error) {
      lastError =
...
): TOpenFoodFactsCandidate | null {
  const productName = getString(product.product_name_en) ?? getString(product.product_name);
  if (!productName) return null;

  const brand = getString(product.brands)?.split(",")[0]?.trim() || null;
  const barcode = getString(product.code) ?? null;
  const nutriments = (product.nutriments ?? {}) as Record<string, unknown>;

  return {
    barcode,
    brand,
    categories: toStringArray(product.categories_tags, product.categories),
    countries: toStringArray(product.countries_tags, product.countries),
    imageUrl: getString(product.image_front_small_url) ?? null,
    normalizedName: normalizeText([brand, productName].filter(Boolean).join(" ")),
    nutrimentsQualityWarnings: toStringArray(product.data_quality_errors_tags),
    nutrientsPer100g: mapNutrients(nutriments),
    productName,
    quantity: getString(product.quantity) ?? null,
    servingSize: getString(product.serving_size) ?? null,
    source: "open_food_facts",
    ukMarketMatch: isUkRelevant(product),
  };
}

function mapNutrients(nutriments: Record<string, unknown>): TFoodNutrients {
  return compact({
