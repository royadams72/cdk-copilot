...
- `steps` from Health Connect step records and aggregate totals
...
- `source: "provider"`
- `provider.packageName`: the Health Connect data origin package
- `externalRecordId`: a Health Connect record id/client id when available, or a deterministic fallback key

For the daily displayed step summary, `externalRecordId` is generated by the app as `health-connect:{packageName}:steps:{YYYY-MM-DD}` because that row represents a daily aggregate rather than one raw upstream record. For heart rate, blood pressure, sleep, and exercise records, the app prefers Health Connect `metadata.id` or `metadata.clientRecordId`. These keys prevent repeated syncs from creating duplicate documents. Manual exercise, sleep, heart rate, and blood pressure entries continue to save as `source: "patient"` unless explicit provenance is supplied.

Changing Health Connect permissions in `app.json` or `AndroidManifest.xml` requires a native rebuild and reinstall of the dev client.
