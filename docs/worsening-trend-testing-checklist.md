# Worsening Trend Testing Checklist
...
- The next thing I would question is performance on the portal patient list, because it now computes active worsening episodes per patient instead of reading only flags. It is the right behavior, but if the accessible patient count grows a lot, that route should be batched or precomputed.
